    ...
    public partial class FormPT : Form
    {
        ...
        private void tbsearch_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(tbsearch.Text.ToString()))
                {
                    ...
                    query = "SELECT ID_PAKET_TOUR, nama_paket_tour,nama_kota,foto from daerah d, paket__tour p where d.ID_DAERAH = p.ID_DAERAH and NAMA_KOTA = '" + cbdaerah.SelectedValue.ToString() + "'and ID_PAKET_TOUR like 'PT%';";
                    sqlCommand = new MySqlCommand(query, sqlConnect);
                    sqlAdapter = new MySqlDataAdapter(sqlCommand);
                ...
                else
                {
                    ...
                    sqlConnect = new MySqlConnection(stringConnect);
                    string query2 = "SELECT NAMA_PAKET_TOUR ,ID_PAKET_TOUR , nama_kota,foto from paket__tour p, daerah d where p.ID_DAERAH = d.ID_DAERAH and NAMA_KOTA = '" + cbdaerah.SelectedValue.ToString() + "' and NAMA_PAKET_TOUR like '" + tbsearch.Text.ToString() + "%'and ID_PAKET_TOUR like 'PT%';";
        ...
        private void cbdaerah_SelectedValueChanged(object sender, EventArgs e)
        {
            try
            {
                this.panelpaket.Controls.Clear();
                DataTable hitung1 = new DataTable();
                query = "SELECT HARGA_TOUR_ORG, ID_PAKET_TOUR, nama_paket_tour,nama_kota,foto from daerah d, paket__tour p where d.ID_DAERAH = p.ID_DAERAH and NAMA_KOTA = '" + cbdaerah.SelectedValue.ToString() + "'and ID_PAKET_TOUR like 'PT%';";
                sqlCommand = new MySqlCommand(query, sqlConnect);
                sqlAdapter = new MySqlDataAdapter(sqlCommand);
                sqlAdapter.Fill(hitung1);
