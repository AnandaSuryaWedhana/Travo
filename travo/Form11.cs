    ...
    public partial class FormAdminKategori : Form
    {
        ...
        private void FormAdminKategori_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                sqlConnect = new MySqlConnection(connectString);
                SqlQuery = "SELECT ID_KATEGORI as `ID kategori`,JENIS_KATEGORI as `Jenis Kategori` FROM kategori";
                SqlCommand = new MySqlCommand(SqlQuery, sqlConnect);
                SqlAdapter = new MySqlDataAdapter(SqlCommand);
                SqlAdapter.Fill(dtkategori);
                cbkategoriupdate.DataSource = dtkategori;
                cbkategoriupdate.DisplayMember = "Jenis Kategori";
                cbkategoriupdate.ValueMember = "ID Kategori";
        ...
        private void DGV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                SqlQuery = "SELECT ID_KATEGORI as `ID kategori`,JENIS_KATEGORI as `Jenis Kategori` FROM kategori WHERE ID_KATEGORI ='" + tbidupdate.Text.ToString() + "'";
                sqlConnect = new MySqlConnection(connectString);
                SqlCommand = new MySqlCommand(SqlQuery, sqlConnect);
                SqlAdapter = new MySqlDataAdapter(SqlCommand);
                SqlAdapter.Fill(Dtkategoriupdate);
                cbkategoriupdate.SelectedValue = Dtkategoriupdate.Rows[0][0].ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void btupdate_Click(object sender, EventArgs e)
        {
            try
            {
                SqlQuery = "UPDATE kategori SET  JENIS_KATEGORI = '" + cbkategoriupdate.SelectedIndex.ToString() + "', DURASI_KATEGORI ='" + tbdurasiupdate.Text.ToString() + "',DELETE_KATEGORI = 0 WHERE ID_KATEGORI = '" + tbidupdate.Text.ToString() + "';";
